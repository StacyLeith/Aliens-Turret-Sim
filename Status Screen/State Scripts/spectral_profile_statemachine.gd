...
onready var sysmode_state = $specpro_states/bio
# Sets the mode when the scene is loaded
onready var next_sysmode_state = $specpro_states/inert
# Sets the next mode to select

var current_state_set = "bio"

func _ready(): 
	# Function fires when the script has fully loaded
	$BTN_SPECTRAL_PROFILE.connect("pressed", self, "_change_state") 
...
func _change_state():
	...
	if sysmode_state == $specpro_states/bio:
		current_state_set = "bio"
		next_sysmode_state = $specpro_states/inert
	else:
		current_state_set = "inert"
		next_sysmode_state = $specpro_states/bio
